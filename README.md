...
```python
RAMP_RATE_UP_PER_SEC:   Optional[float] = 8   # 上昇の最大レート (%/秒)
...
| `RAMP_RATE_UP_PER_SEC` | `8` | ファン速度上昇の最大レート（%/秒）。`None` で即時 |
| `RAMP_RATE_DOWN_PER_SEC` | `2` | ファン速度下降の最大レート（%/秒）。`None` で即時 |
...
<details>
<summary><strong>ファンの反応が鈍い / 温度が上がっても速度がすぐ上がらない</strong></summary>

ランプレート制御により、ファン速度は時間をかけて目標値に追従します。急な負荷変動に即応させたい場合は、`RAMP_RATE_UP_PER_SEC` を大きくする（例: 15〜20）か、`None` に設定して即時変化に戻してください。
...
│  ┌──────────────┐                                       │
│  │  温度を取得  │ ─→ 失敗なら FAILSAFE_SPEED (即時適用) │
│  └──────┬───────┘                                       │
│         ↓                                               │
│  ┌──────────────┐                                       │
│  │  カーブ補間  │ → 目標ファン速度を算出                │
